...
window.addEventListener('load', function() {
    ...
    function startGame() {
        currentPlayerName=playerNameInput.value.trim(); if(!currentPlayerName){alert("Veuillez entrer votre nom !"); return;} if(currentPlayerName.length>15){currentPlayerName=currentPlayerName.substring(0,15);} localStorage.setItem('uniteamPlayerName', currentPlayerName); console.log("Player Name:", currentPlayerName);
    ...
    async function submitScore(name, scoreValue) {
         if (!window.db || typeof firebase === 'undefined' || !firebase.firestore) { console.error("Firestore non initialisé."); return; }
         const { collection, doc, getDoc, setDoc } = firebase.firestore;
         if (!name || typeof scoreValue !== 'number' || scoreValue < 0) { console.log("Nom/score invalide."); return; }
         console.log(`Envoi: ${name} - ${scoreValue}`);
         const scoresRef = collection(window.db, 'scores'); const playerRef = doc(scoresRef, name);
         try { const docSnap = await getDoc(playerRef);
             if (docSnap.exists()) { const best = docSnap.data().score || 0; if (scoreValue > best) { console.log(`Nouveau record pour ${name}: ${scoreValue}`); await setDoc(playerRef, { name: name, score: scoreValue }); } else { console.log(`Score ${scoreValue} <= ${best}`); } }
             else { console.log(`Nouveau joueur: ${name} score ${scoreValue}`); await setDoc(playerRef, { name: name, score: scoreValue }); }
         } catch (error) { console.error("Erreur Firestore écriture:", error); throw error; }
    }
    async function displayLeaderboard() {
        if (!leaderboardListElement) return; leaderboardListElement.innerHTML = '<li>Chargement...</li>';
         if (!window.db || typeof firebase === 'undefined' || !firebase.firestore) { console.error("Firestore non initialisé."); leaderboardListElement.innerHTML = '<li>Erreur connexion.</li>'; return; }
        const { collection, query, orderBy, limit, getDocs } = firebase.firestore;
        try { const scoresRef = collection(window.db, 'scores'); const q = query(scoresRef, orderBy('score', 'desc'), limit(10)); const querySnapshot = await getDocs(q);
            if (querySnapshot.empty) { leaderboardListElement.innerHTML = '<li>Aucun score.</li>'; return; }
            let html = ''; let rank = 1;
            querySnapshot.forEach((doc) => { const d = doc.data(); const n = d.name.replace(/</g, "&lt;").replace(/>/g, "&gt;"); html += `<li>${rank}. ${n} <strong>${d.score}</strong></li>`; rank++; });
            leaderboardListElement.innerHTML = html;
        } catch (error) { console.error("Erreur Firestore lecture:", error); leaderboardListElement.innerHTML = '<li>Erreur chargement.</li>'; }
    }
